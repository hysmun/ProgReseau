...
{
 char Tampon[80] ;
 sprintf(Tampon,"ref = %d",UnRecord->Reference ) ;  MonPrintf(Tampon,4,strlen(Tampon)) ;
 sprintf(Tampon,"film = %s",UnRecord->Film) ;    MonPrintf(Tampon,50,strlen(Tampon)) ;
 sprintf(Tampon, " rea = %s", UnRecord->Realisateur);   MonPrintf(Tampon,30,strlen(Tampon)) ;
 sprintf(Tampon,"place = %d",UnRecord->Places) ; MonPrintf(Tampon,4,strlen(Tampon)) ;
 printf("\n") ;
 fflush(stdout);
}
...
char *ipString(struct sockaddr_in sor, char *ctmp)
{
	...
	sprintf(ctmp, "%s:%u",ctmp, ntohs(sor.sin_port) );
	return ctmp;
}
